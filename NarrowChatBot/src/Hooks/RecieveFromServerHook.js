					...
					if (hookArgs[0].currentTarget.url.includes("narrow.one/ws")) {
						...
						if (s[0] == ReceiveAction.CHAT_MESSAGE) {
							...
							if (msg) {
								...
								if (true) { // !n.playerName.includes("YeemiRouth")
									if (packet.data.message.startsWith("/")) {
										const [command, ..._args] = packet.data.message.slice(1).split(" ");
										const args = { ..._args };

										switch (command.toLowerCase()) {
											case "help":
												SendChatAsBot("/help - get a list of bot commands");
												SendChatAsBot("/hello - test command");
												SendChatAsBot("/elo <player> - get a players elo");
												SendChatAsBot("/pros - get a list of pro players ingame");
												break;

											case "elo":
												if (args[0] === undefined) {
													SendChatAsBot("your elo is " + Math.round(n.elo));
													break;
												}

												let found = false;
												for (const player of NarrowSDK.Main.gameManager.activeGame.players.values()) {
													if (player.playerName.includes(args[0])) {
														SendChatAsBot("elo " + Math.round(player.elo));
												...
												if (!found) {
													SendChatAsBot("cant find player ingame");
												}
												break;
											...
											case "pros":
												...
												for (const player of NarrowSDK.Main.gameManager.activeGame.players.values()) {
													if (player.elo >= 1) {
														SendChatAsBot(player.playerName + " has " + Math.round(player.elo) + " elo");
												...
												if (!found2) {
													SendChatAsBot("cant find anyone ingame with high score");
												}
												break;

											case "hello":
												SendChatAsBot("Hello, " + n.playerName + "!");
												break;

											default:
												SendChatAsBot("Unknown command: " + command);
												break;
										}
									} else {
